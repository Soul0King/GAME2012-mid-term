...
Vector2 topMid = Vector2Lerp({ 1.0f,  1.0f }, { -1.0f,  1.0f }, 0.5f);
static const int line_vertex_count = 6;
...
int main()
{
    ...
    pointPositions[0] = Vector2{ Random(-1.0f, 1.0f),  Random(-1.0f, 1.0f) };
    for (int i = 1; i < pointPositions.size(); i++)
    {
        ...
        pointPositions[i] = Vector2Lerp(pointPositions[i - 1], rv, 0.5f);
